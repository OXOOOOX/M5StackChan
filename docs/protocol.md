...
归档前补查发现：遥控器独立仓库的后续 `b61fae0` 版本使用带组件封装的 `espnow_send(ESPNOW_DATA_TYPE_DATA, ...)`，没有保留本库旧遥控器分支 `2ce2b47` 的原生发送改动。其显示、摇杆和反转修复成功不等于与当前 UIFlow2 接收器联调通过。配套版本和恢复入口见[归档交接](project_handoff_zh.md)。
