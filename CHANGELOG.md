...
- 复核历史聊天成果、已发布文件、忽略文件与子模块，补充归档后的分仓恢复入口和未完成事项。
- 确认遥控器独立仓库发布补丁遗漏两个反转界面源码，已通过该库 `def7e75` 补全上传；记录干净基线应用验证的边界。
- 明确旧遥控分支的原生 ESP-NOW 与独立仓库后续组件封装的差异，避免将最新显示/反转固件视为 UIFlow2 接收器已验收配套。
- 具体发布位置、环境横幅与 EzData 历史入口见[归档交接](docs/project_handoff_zh.md)。
